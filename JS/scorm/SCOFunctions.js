...
*******************************************************************************/
function marcaPosicao(p,t) {
   doLMSSetValue( "cmi.core.lesson_location", p );
   if(p==t){
...
function marcaScore(score) {
   doLMSSetValue( "cmi.core.score.raw", score );
   doLMSCommit();
} 

function marcaAnotacoes(anota) {
   doLMSSetValue( "cmi.suspend_data", anota );
   doLMSCommit();
}

function pegaTempoMax(){
	var tempo = doLMSGetValue( "cmi.student_data.max_allow_time" );
	alert(tempo);
}
